...
def post_editor():
    ...
    data['authors'] = json.dumps(data.get('authors', [g.user.username]))
    data['tags'] = json.dumps(data.get('tags', []))

    if g.user.username not in data.get('authors', []) or g.user.username in current_repo.config.editors:
        data['can_approve'] = 1
...
def save_post():
    ...
    path = '{}/{}'.format(data['repo'], data['path'])

    # TODO how to prevent undesired overwriting
...
def unpublish_post():
    """ Unpublish the post """
    path = request.args.get('path', None)
    if not path not in current_repo:
        return json.dumps({'msg': "Unable to retrieve post with path = {}!".format(path), 'success': False})
...
def delete_post():
    ...
    if path not in current_repo:
        return json.dumps({'msg': "Unable to retrieve post with path = {}!".format(path), 'success': False})
    if g.user.username in current_repo.post(path).headers['authors']:
